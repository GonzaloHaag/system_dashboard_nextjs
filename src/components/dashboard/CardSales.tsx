...
import { getTotalCountVentas } from "@/actions";
...
export const CardSales = ({ userId }: { userId: number }) => {

    const [totalCount, setTotalCount] = useState(0);
    const [isLoading, setIsLoading] = useState(true);
    useEffect(() => {
        const fetchCountSales = async () => {
            try {
                setIsLoading(true);
                const response = await getTotalCountVentas(userId);
                if (!response.ok) {
                    toast.error(response.message);
                    return;
                }
    
                setTotalCount(response.totalCountSales || 0)
